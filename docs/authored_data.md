...
The BSP29 Reader is intentionally not connected to `Level::Loader`, because the current loader constructs grid-backed `Level` objects and BSP static-world structure should not be forced through that representation. `Render::BSP29World` consumes world model `0` directly from `BSP29::MapData`, while BSP bootstrap/collision consume other normalized BSP structures according to their own needs.
