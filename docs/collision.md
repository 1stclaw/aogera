...
Current player and NPC locomotion both use this query through one `Simulation::GroundMovement`. Player input supplies per-tick `GroundMove` directly; NPC `GroundMove` is generated by `Simulation::GroundSteering` every fixed tick after `Simulation::GroundNavigation` selects a local `GroundHeading` from the current `SteeringTarget`. In BSP mode, the same `GroundSpace` sends all positive-radius static traces through `BSP29::GroundClearance`, while continuous `GroundBody` collision remains authoritative for dynamic actors:
...
- `BSP29::GroundClearance` supplies compiled hull-1 traces for actor movement and local-navigation probes;
...
Active BSP chase now shares the same continuous collision authority as movement.

```text
SteeringTarget(goal entity / world point)
        |
        v
GroundNavigation
        |
        | sweep_circle probes from actual Position
        v
GroundHeading
        |
        v
...
`Simulation::GroundNavigation` does not inspect grid cells or BSP structures directly. It asks `GroundSpace#sweep_circle` whether short candidate headings are usable for the actor's current `Position` and `GroundBody`. Direct pursuit is preferred; blocked movement can fall back to the previous heading, blocking-plane tangents, or deterministic rotated alternatives.
...
local navigation         -> GroundNavigation via GroundSpace sweeps
local heading state       -> GroundHeading
dynamic actor collision   -> GroundBody circles
```

`App` now supplies one BSP-backed `GroundSpace` to `Mode::Play`, `RealtimeController`, `Simulation::GroundNavigation`, and `Simulation::Executor`. Positive-radius static movement and local-navigation probes therefore consult the same BSP29 compiled-hull collision source. `Simulation::Executor` no longer needs a separate bound-character movement service.
