...
Current player and NPC locomotion both use this query through one `Simulation::GroundMovement`. Player input supplies per-tick `GroundMove` directly; NPC `GroundMove` is generated by `Simulation::GroundSteering` every fixed tick from persistent `SteeringTarget` intent. In BSP mode, the same `GroundSpace` sends all positive-radius static traces through `BSP29::GroundClearance`, while continuous `GroundBody` collision remains authoritative for dynamic actors:
...
The grid step is not exposed to steering. `Simulation::Pathfinder#next_waypoint` converts an ordinary chosen planning cell to a world-space `Pathfinder::Waypoint`; when the source already occupies a target-adjacent goal cell, it can instead return the target's current X/Z for the continuous final approach, provided BSP static ground clearance allows that direct segment. `RealtimeController` consumes only the resulting world-space target.
