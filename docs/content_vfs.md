...
This source choice does not propagate into `App`, simulation, collision, or rendering. They receive decoded data rather than source handles. For PAK-backed BSP runtime launch, the CLI also reads `gfx/palette.lmp` through the same VFS and passes the decoded `Quake::Palette` alongside the decoded BSP map. `Content::Directory` is implemented and tested as a VFS source but does not yet have a dedicated BSP CLI option.

The same byte boundary is used for Quake's palette data. `Quake::PaletteReader` accepts bytes only; PAK-backed spectator/walkthrough launch reads `gfx/palette.lmp` through `Content::VFS` and passes those bytes to the decoder. `Quake::MipTextureDecoder` then operates only on an already-parsed `BSP29::MipTexture` plus the decoded palette; neither decoder knows about PAKs or VFS mounts. Rendering receives the decoded palette as ordinary data, expands only the used BSP base textures for GPU upload, and keeps the separately uploaded original low-resolution lightmap atlas independent. Inspection commands still need only BSP bytes and therefore do not require palette content.
